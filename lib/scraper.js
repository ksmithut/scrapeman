...
Scraper.prototype.pause = function () {
  ...
  Object.keys(self._requests).map(function (url) {
    self._requests[url].abort();
  });
...
Scraper.prototype.crawl = function () {
  ...
  self._requests[item.to] = request({
    url: item.to,
    method: 'GET',
    maxRedirects: 10,
    pool: { maxSockets: 11 },
    ...
    self._processResource(err, self._resources[item.to], res);
  });
};
...
Scraper.prototype._processResource = function (err, thisResource, res) {
  ...
  // the queue again
  /* istanbul ignore next: This shouldn't happen now because we kill requests */
  if (!self._started) { return; }

  self._pending--;
  thisResource.pending = false;
  delete thisResource.pending;
  delete self._requests[thisResource.to];
  ...
  // This next little block is all about handling redirecting...
  // TODO: should refactor this a little bit
  var item = thisResource;
  var skip = false;
  var actualUrl    = res.request.uri.href;
  var requestedUrl = thisResource.url;
  // if it was redirected...
  if (actualUrl !== requestedUrl) {

    if (!self._resources[actualUrl]) {
      self._resources[actualUrl] = {
        from: [],
        url: requestedUrl,
      };
    } else {
    ...
    self._resources[actualUrl].from.push(requestedUrl);
    self._resources[actualUrl].url = actualUrl;
    thisResource.redirectTo = actualUrl;

    item = self._resources[actualUrl];
  ...
  self._plugins.map(function (pluginCb) {
    try {
      pluginCb.call(self, item, res, self._resources, self._data);
    } catch (err) {
      self.emit('pluginError', err, item, res);
    }
  });
  ...
  this.emit('resourceProcessed', self, item);
};
